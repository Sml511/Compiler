...
struct token* token_make_multilipe_comment()
{
    ...
    while(1)
    {
        ...
        if(c==EOF)
        {
            ...
            compiler_error(lex_process->compiler,"you did not close this multilipe comment\n");
...
struct token* handle_comment()
{
    ...
    if(c == '/')
    {
        ...
        else if(peekc() == '*')
        {
            ...
            nextc();
            return token_make_multilipe_comment();
...
void lexer_validate_binary_string(const char* str)
    {
        ...
        for(int i=0; i<len; i++)
        {
            ...
            if(str[i] != '0'&& str[i]!='1')
            {
                ...
                compiler_error(lex_process->compiler,"This is");
...
struct token* token_make_speical_number_binary()
        {
...
struct token* token_make_special_number()
        {
            ...
            }else if(c == 'b')
            {
                ...
                token = token_make_speical_number_binary();
...
struct token* token_make_quote()
        {
            ...
            if(nextc() != '\'')
            {
                ...
                compiler_error(lex_process->compiler,"you need a quote");
...
struct lex_process_functions lex_string_buffer_functions =
        {
...
struct lex_process* tokens_build_for_string(struct compile_process* compiler,const char* str)
        {
                ...
                struct lex_process* lex_process = lex_process_create(compiler,&lex_string_buffer_functions,buffer);
                if(!lex_process)
                {
