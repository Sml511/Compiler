...
static void parser_ignore_nl_or_comment(struct token* token)
        {
            ...
            while(token && parser_is_nl_or_comment_or_newline_seperator(token))
            {
...
void parse_single_token_to_node()
{
        ...
        switch(token->type)
        {
            ...
            case TOKEN_TYPE_NUMBER:
                ...
                node = node_create(&(struct node){.type=TOKEN_TYPE_NUMBER,.llnum=token->llnum});
            ...
            default:
                compiler_error(current_process,"This is not a Single token that can not be converted to a node");
